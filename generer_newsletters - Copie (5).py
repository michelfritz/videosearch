...
if not SMTP_HOST and MAILJET_API_KEY and MAILJET_SECRET_KEY:
    SMTP_HOST = "in-v3.mailjet.com"  # relais SMTP Mailjet :contentReference[oaicite:1]{index=1}
...
- Ajoute 3 à 6 badges (div.badges). Ajoute un bloc div.callout (idée forte).
- Termine par un CTA (lien) (ex: "Voir le replay").
- RENDS UNIQUEMENT LE HTML DU CONTENU (sans <html>/<body>) ET SANS BLOCS DE CODE.
Réécris uniquement le contenu éditorial en HTML (sans style ni <body>), sur la base de la transcription suivante :"""

def extract_h1(html: str) -> str:
    if not html: return "Newsletter A La Lucarne"
    m = re.search(r"<h1[^>]*>(.*?)</h1>", html, re.I|re.S)
    if not m: return "Newsletter A La Lucarne"
...
def keywords_from_prompt(prompt: str, max_terms: int = 6) -> str:
    STOP = {"le","la","les","un","une","des","de","du","d","et","ou","au","aux","en","dans","sur","pour","avec","sans","par","vers","chez","se","sa","son","ses","ce","cet","cette","ces","que","qui","quoi","dont","où","quand","comme","plus","moins","très","tres","bien","a","à","the","of"}
...
def generer_openai_image(prompt: str, out_path: Path):
    ...
    enriched = (
        prompt + " — editorial, photorealistic office scene with real people, faces clearly visible, natural lighting, professional color grading; no illustration, no cartoon, no 3d, no cgi, no render, no watermark, no text"
    )
    sizes = ["1536x1024", "1024x1024", "1024x1536"]
    for sz in sizes:
        try:
            im = client.images.generate(model="gpt-image-1", prompt=enriched, size=sz, quality="high", n=1)
...
def assemble_html(full_html_inner: str, title_fallback: str, hero_file: Optional[Path], image_source: str = "") -> str:
    ...
    if hero_file and hero_file.exists():
        ...
        hero_tag = (f'<div class="hero"><img src="{hero_src}" alt="Hero" data-source="{image_source}"><div class="overlay"></div>'
...
<div class="container">
  ...
  <div class="inner">
    {h1_block}
    {inner_no_h1}
    <div class="footer">Newsletter – A La Lucarne • {formatdate(localtime=True)}</div>
  </div>
</div>"""
...
def send_email(subject: str, html: str, recipients: list[str], hero_bytes: Optional[bytes]):
    ...
    msg_root["Subject"] = subject; msg_root["From"] = SMTP_FROM
    msg_root["To"] = ", ".join(recipients); msg_root["Date"] = formatdate(localtime=True)
    alt = MIMEMultipart("alternative"); msg_root.attach(alt)
    text = html_to_text(html); alt.attach(MIMEText(text, "plain", "utf-8"))
    if hero_bytes:
        cid = make_msgid(domain="alalucarne.com")[1:-1]
        img = MIMEImage(hero_bytes); img.add_header("Content-ID", f"<{cid}>")
        img.add_header("Content-Disposition", "inline", filename="hero.png")
        msg_root.attach(img)
        html = re.sub(r'src="images/[^"]+"', f'src="cid:{cid}"', html)
    alt.attach(MIMEText(html, "html", "utf-8"))
    with smtplib.SMTP(SMTP_HOST, SMTP_PORT) as s:
        s.starttls(); s.login(SMTP_USER, SMTP_PASS); s.sendmail(SMTP_FROM, recipients, msg_root.as_string())
...
def main():
    ...
    for fichier_txt in fichiers:
        ...
        # Marquer comme fait
        mark_done(SCRIPT_NAME, nom, str(fichier_txt), fp, str(out_html), extra={"image_source": image_source})
