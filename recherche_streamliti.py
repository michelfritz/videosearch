    ...
    if user_question:
        try:
            ...
            with st.spinner("Recherche intelligente en cours..."):
                ...
                # 1) Optionnel: FAISS → désactivé si FORCE_NUMPY
                if not FORCE_NUMPY:
                    try:
                ...
                # 2) Fallback NumPy (ou forcé)
                if docs is None:
                    if DEBUG_MODE: st.write("DEBUG: Fallback numpy → embed_openai()")
                    vq = embed_openai(user_question)  # SI ça bloque ici → clé ou réseau
                    if DEBUG_MODE: st.write("DEBUG: vecteur query OK → rechercher_similaires()")
                ...
                if not context.strip():
                    ...
                    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.2, openai_api_key=get_openai_key())
...
""".strip()
            ...
            st.exception(e)  # affiche stack trace utile
    else:
