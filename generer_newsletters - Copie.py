...
# ----------- CSS (puces classiques + hero plus bas + cadrage visages) -------------
BLOC_CSS = """
<style>
:root {
...
# ----------- Image: OpenAI photorealistic (faces visibles) + Unsplash secours -------------
FRENCH_STOPWORDS = {
...
def send_email(subject: str, html: str, recipients: list[str], hero_bytes: Optional[bytes]):
    ...
    if hero_bytes:
        ...
        msg_root.attach(img)
        html_email = re.sub(r'src="images/[^"]+"', f'src="cid:{cid}"', html)
    else:
        html_email = html
    ...
    print("[OK] Email sent to:", ", ".join(recipients))

# ----------- MAIN ---------------------------
def main():
    print("[INFO] Looking for TXT resumes in:", DOSSIER_RESUME)
    fichiers = list(Path(DOSSIER_RESUME).glob("*.txt"))
    print("[INFO] Found:", [f.name for f in fichiers])

    for fichier_txt in fichiers:
        nom = fichier_txt.stem
        print("\n[RUN] Building newsletter for:", nom)

        texte_resume = fichier_txt.read_text(encoding="utf-8", errors="ignore").strip()
        if not texte_resume:
            print("[WARN] Empty file, skipping:", nom)
            continue
        ...
        if hero_file and hero_file.exists():
            ...
            print(f"[OK] Hero image ready from: {image_source}")
        else:
            print("[INFO] No hero image generated.")
            image_source = ""
        ...
        print("[OK] Newsletter saved:", out_html)

        subject = extract_h1(inner_html) or f"Newsletter - {nom}"
        try:
            send_email(subject, final_html, RECIPIENTS, hero_bytes)
        except Exception as e:
            print("[WARN] Email send error:", e)

    print("\n[OK] All newsletters generated.")

if __name__ == "__main__":
    from pathlib import Path  # sûreté import
    main()
