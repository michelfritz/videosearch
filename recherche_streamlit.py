...
def rechercher_similaires(vecteur_query, vecteurs, top_k=5):
    similarities = np.dot(vecteurs, vecteur_query.squeeze())
...
if query:
    ...
    for idx, score in zip(indices, scores):
        bloc = df.iloc[idx]
        start_time = int(bloc['start'])
        video_url = f"https://www.youtube.com/embed/t21LM4CXaqE?start=import os
os.environ["STREAMLIT_WATCHER_TYPE"] = "none"
...
{start_time}&autoplay=1"
        st.markdown(f"**⏱️ Timestamp**: `{start_time}`")
