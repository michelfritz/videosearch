# generer_newsletters.py — newsletter vivante + photo hero + incrémentalité
import os
import re
import base64
import smtplib
...
SENT_LOG = Path(DOSSIER_SORTIE) / "sent.log"
os.makedirs(DOSSIER_SORTIE, exist_ok=True)
os.makedirs(DOSSIER_IMAGES, exist_ok=True)

# ----------- Incrémentalité & email -----------
# SKIP_IF_UP_TO_DATE=1 : si le .html existe ET est plus récent que le .txt, on saute (par défaut ON)
# AVOID_RESEND=1       : évite de renvoyer un email pour un même fichier si déjà envoyé (journal sent.log)
# FORCE_REBUILD=1      : régénérer le HTML même s'il est à jour
SKIP_IF_UP_TO_DATE = os.getenv("SKIP_IF_UP_TO_DATE", "1") == "1"
AVOID_RESEND       = os.getenv("AVOID_RESEND", "1") == "1"
...
# Envoi d'email activé seulement si SMTP est complet
SEND_EMAILS = all([SMTP_HOST, SMTP_USER, SMTP_PASS, SMTP_FROM])
...
def generer_newsletter(texte_resume: str) -> str:
    if not client:
        return f"<h1>Newsletter</h1><div class='badges'></div><div class='callout'>Résumé non réécrit.</div><p>{texte_resume[:800]}...</p><a class='cta' href='#'>Voir le replay</a>"
...
# ----------- Image: OpenAI photorealistic + Unsplash -------------
FRENCH_STOPWORDS = {
...
def assemble_html(full_html_inner: str, title_fallback: str, hero_file: Optional[Path], image_source: str = "") -> str:
    ...
    inner_no_h1 = re.sub(r"<h1[^>]*>.*?</h1>", "", full_html_inner, flags=re.IGNORECASE | re.DOTALL).strip()
...
def send_email(subject: str, html: str, recipients: list[str], hero_bytes: Optional[bytes]):
    ...
    if hero_bytes:
        ...
        msg_root.attach(img)
        import re as _re
        html_email = _re.sub(r'src="images/[^"]+"', f'src="cid:{cid}"', html)
    else:
        html_email = html
...
# ----------- MAIN ---------------------------
def _read_sent_log() -> set:
    if not SENT_LOG.exists():
        return set()
    try:
        lines = SENT_LOG.read_text(encoding="utf-8").splitlines()
        return set([ln.split("|",1)[0].strip() for ln in lines if ln.strip()])
    except Exception:
        return set()

def _append_sent_log(stem: str, subject: str):
    try:
        SENT_LOG.parent.mkdir(parents=True, exist_ok=True)
        with SENT_LOG.open("a", encoding="utf-8") as f:
            f.write(f"{stem}|{subject}|{formatdate(localtime=True)}\n")
    except Exception:
        pass
...
def main():
    ...
    deja_envoyes = _read_sent_log() if AVOID_RESEND else set()

    for fichier_txt in fichiers:
        nom = fichier_txt.stem
        out_html = Path(DOSSIER_SORTIE) / f"{nom}.html"

        # Incrémentalité: si HTML existant et plus récent que le TXT, on saute (sauf FORCE_REBUILD)
        if SKIP_IF_UP_TO_DATE and out_html.exists() and not FORCE_REBUILD:
            try:
                if out_html.stat().st_mtime >= fichier_txt.stat().st_mtime:
                    print(f"[SKIP] À jour: {nom} (HTML plus récent que le TXT)")
                    continue
            except Exception:
                pass
        ...
        if not texte_resume:
            print("[WARN] Fichier vide, saut:", nom)
            continue
        ...
        # Envoi email si pas déjà envoyé (selon sent.log) et SMTP configuré
        if SEND_EMAILS:
            if AVOID_RESEND and nom in deja_envoyes:
                print("[SKIP] Email déjà envoyé pour:", nom)
            else:
                subject = re.sub(r"<.*?>", "", extract_h1(inner_html)) or f"Newsletter - {nom}"
                try:
                    send_email(subject, final_html, RECIPIENTS, hero_bytes)
                    if AVOID_RESEND:
                        _append_sent_log(nom, subject)
                except Exception as e:
