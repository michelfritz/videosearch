...
def _sanitize_b2c_text(s: str) -> str:
    ...
    # retirer lignes internes (visio/webinar/vidéo/YouTube/formation interne)
    banned = r"(webinar|visi[oô]|formation\s+interne|atelier|module|newsletter|youtube|vid[ée]o)"
    lines = [ln for ln in s.splitlines() if not re.search(banned, ln, flags=re.I)]
    s = "\n".join(lines)
    # adoucir le vocabulaire interne
...
def _compose_caption_variant(topic_key: str, title_hint: str, facts: List[str], city: Optional[str], platform: str, persona: str) -> str:
    ...
    if platform == "ig":
        ...
        body = f"{hook} — {body}{_cta_for_persona(persona, city)}\n\n"
        return _sanitize_b2c_text(body + " ".join(ig_hash))
    else:
        ...
        return _sanitize_b2c_text(txt + " ".join(fb_hash))
...
def generate_captions(text: str, title_hint: str, video_url: str) -> dict:
    ...
    payload = CAPTION_PROMPT.format(
        title_hint=title_hint, topics_inline=topics_inline, facts_inline=facts_inline, text=text[:4000]
    )
    try:
        ...
        if m:
            ...
            j["ig"] = [_sanitize_b2c_text((x or "").strip()) for x in j.get("ig", [])][:2]
            j["fb"] = [_sanitize_b2c_text((x or "").strip()) for x in j.get("fb", [])][:2]
            j["title"] = (j.get("title") or title_hint or "À La Lucarne")[:80]
            # si pas assez de variantes, compléter avec fallback local diversifié
            while len(j["ig"]) < 2:
               aud = audA if len(j["ig"]) == 0 else audB
               j["ig"].append(_compose_caption_variant(topic_keys[min(len(j['ig']), len(topic_keys)-1)], title_hint, facts, city_for_hash, "ig", aud))
            while len(j["fb"]) < 2:
               aud = audA if len(j["fb"]) == 0 else audB
               j["fb"].append(_compose_caption_variant(topic_keys[min(len(j['fb']), len(topic_keys)-1)], title_hint, facts, city_for_hash, "fb", aud))
            # Retirer toute URL résiduelle (sécurité)
            j["ig"] = [re.sub(r"https?://\S+", "", s).strip() for s in j["ig"]]
            j["fb"] = [re.sub(r"https?://\S+", "", s).strip() for s in j["fb"]]
            return j
    except Exception as e:
...
def main():
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Drafts"):
        ...
        _write_if_missing(droot / "fb_caption_A.txt", fb_caps[0] if len(fb_caps)>0 else title)
        _write_if_missing(droot / "fb_caption_B.txt", fb_caps[1] if len(fb_caps)>1 else title)
